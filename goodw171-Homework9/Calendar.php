...
// If one or more form entry was blank, set a flag and redirect back to Form.php
...
if($no_empty_values){
	...
	$input = array('Name_of_Event' => "$event_name", 'starttime' => "$start_time", 'endtime' => "$end_time", 'location' => "$location");
    ...
    if(!empty($json)){
    	foreach($json as $key=>$values){
    		foreach($values as $value){
    			$markerName_of_Event = $value['Name_of_Event'];
    			$markerPlace = $value['location'];
          ?>
            var Address_of_Event = "<?php echo $markerPlace; ?>";
            var Name_of_Event = "<?php echo $markerName_of_Event; ?>";
            geocodePersonalAddress(geocoder, map, Address_of_Event, Name_of_Event);
          <?php
    		}
    	}
    }
...
function geocodePersonalAddress(geocoder, resultsMap, Address_of_Event, Name_of_Event) {
  geocoder.geocode({'address': Address_of_Event}, function(results, status) {
	if (status === 'OK') {
		var marker = new google.maps.Marker({
  		map: resultsMap,
  		position: results[0].geometry.location,
